...
@dag(dag_id="mal_payment_pipeline", schedule="@daily",
     start_date=datetime(2025, 3, 1), catchup=False, tags=["payments", "mal"])
def payment_pipeline():
    ...
    def load(validated: dict[str, Any]) -> dict[str, int]:
        ...
        if valid:
            p = dlt.pipeline(pipeline_name="mal_payments",
            ...
            con.close()
        return {"valid": len(valid), "errors": len(errors),
    ...
    def run_analytics(load_stats: dict[str, int]) -> dict[str, Any]:
        ...
        queries = [q.strip() for q in (SQL_DIR / "queries.sql").read_text().split(";")
                   if q.strip() and not q.strip().startswith("--")]
        results = {}
        for query in queries:
            label = [l for l in query.split("\n") if l.strip().startswith("--")]
