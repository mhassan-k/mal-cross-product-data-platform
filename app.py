...
def load_data():
    con = duckdb.connect()
    con.execute(
        f"CREATE VIEW events AS SELECT *, CAST(event_timestamp AS DATE) AS event_date, "
...
sources = st.sidebar.multiselect(
    "Source System", all_df["source_system"].unique(), default=all_df["source_system"].unique()
)
...
where = (
    f"source_system IN ({','.join(repr(s) for s in sources)}) "
...
cross = (
    ...
    .groupby("customer_id")
    .agg(products=("payment_type", "nunique"), txns=("event_id", "count"), total_spend=("amt", "sum"))
    .query("products > 1")
...
with st.expander("🔧 SQL Query Runner"):
    sql = st.text_area(
        "SQL (table: `events`):",
        value="SELECT payment_type, COUNT(*) AS cnt, "
        "ROUND(AVG(amt),2) AS avg_amt FROM events GROUP BY 1 ORDER BY cnt DESC",
        height=80,
    )
