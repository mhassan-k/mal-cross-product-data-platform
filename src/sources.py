...
def _base(system: str, src_id: str, cust: str, amt: str, cur: str, ts: datetime,
           status: str, ptype: str, method: str, meta: dict, **extra) -> dict[str, Any]:
    return {"event_id": str(uuid.uuid4()), "source_system": system,
            "source_event_id": src_id, "customer_id": cust,
            "counterparty_id": extra.get("cp_id"), "counterparty_name": extra.get("cp_name"),
            "amount": Decimal(amt), "currency": cur.upper(), "event_timestamp": ts,
            "status": status, "payment_type": ptype, "payment_method": method,
            "metadata": meta, "schema_version": 2}

def _transform_card(r: dict[str, str]) -> dict[str, Any]:
    return _base("cards", r["txn_id"], r["customer_id"], r["txn_amount"], r["txn_currency"],
                 datetime.fromisoformat(r["txn_timestamp"].replace("Z", "+00:00")),
                 STATUS_MAP_CARDS[r["txn_status"]], "card_transaction", r["card_type"],
                 {"card_number": r["card_number"], "mcc_code": r["mcc_code"]},
                 cp_name=r["merchant_name"])

def _transform_transfer(r: dict[str, str]) -> dict[str, Any]:
    return _base("transfers", r["transfer_id"], r["sender_id"], r["amount"], r["ccy"],
                 datetime.fromtimestamp(int(r["created_at"]) / 1000, tz=timezone.utc),
                 STATUS_MAP_TRANSFERS[r["state"]], "transfer", r["transfer_type"],
                 {"reference_note": r["reference_note"]}, cp_id=r["receiver_id"])

def _transform_bill(r: dict[str, str]) -> dict[str, Any]:
    dt = datetime.strptime(r["payment_date"], "%d/%m/%Y %H:%M").replace(tzinfo=timezone.utc)
    return _base("bill_payments", r["payment_id"], r["user_id"], r["pay_amount"],
                 r["currency_code"], dt, STATUS_MAP_BILLS[r["payment_status"]],
                 "bill_payment", r["bill_category"],
                 {"account_number": r["account_number"]},
                 cp_id=r["biller_code"], cp_name=r["biller_name"])

SOURCES = [("cards_events.csv", _transform_card),
