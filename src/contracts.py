...
def migrate_v1_to_v2(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return PaymentEventV2(
        ...
        source_system=v1.source_system,
        source_event_id=v1.source_event_id,
        ...
        payment_method=v1.payment_method,
        metadata={},
        schema_version=2,
    ).model_dump()
...
def ensure_latest_version(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if version == 2:
        return record
    if version == 1:
        return migrate_v1_to_v2(record)
...
def validate_contract(record: dict[str, Any], version: int = 2) -> BaseModel:
    ...
    if model_cls is None:
        raise ValueError(f"No schema registered for version {version}")
